...
-- Description:	Datos de consumos AM a Report InaCatalog
-- Modificacion 18/12/2017
--	   La fecha del reporte es el 1/1 del año actual
-- ========================================================================
CREATE PROCEDURE [dbo].[usp_inaDatosConsumosAM] AS
BEGIN
	SET NOCOUNT ON;
	...
	-- Declarar variables
	declare @codPlantillaReportes int=121 -- Es la plantilla para datos de consumos AM
	...
	-- Periodo de datos
	select @Periodo=' (Ene-' + left(datename(month,DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), -1)),3)
	...
	declare c cursor for
		...
		from	#tmp_Tabla1 t1 
				left outer join [stgF3ConsumosClienteAM] t2
					on	t1.codEmpresa=t2.CodEmpresa
						and t1.CodCliente=t2.CodCliente
						and t2.CodNivel1=1 -- CAFE
				left outer join [stgF3ConsumosClienteAM] t3
					on	t1.codEmpresa=t3.CodEmpresa
						and t1.CodCliente=t3.CodCliente
						and t3.CodNivel1=2 -- COMPLEMENTOS
				left outer join [stgF3ConsumosClienteAM] t4
					on	t1.codEmpresa=t4.CodEmpresa
						and t1.CodCliente=t4.CodCliente
						and t4.CodNivel1=6 -- PLV
				left outer join [stgF3ConsumosClienteAM] t5
