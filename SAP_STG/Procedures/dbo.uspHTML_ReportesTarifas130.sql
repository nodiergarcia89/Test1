...
-- Description: Transformar Reportes 130 a HTML
-- =============================================
CREATE     PROCEDURE [dbo].[uspHTML_ReportesTarifas130] 
AS
BEGIN
    ...
    -- Plantilla HTML
    SELECT @CodPlantillaReportes=130
     ...
     SELECT @html='<html><style>' +
	   ...
	   '<TR><TH colspan="2"><B>Listado de Tarifa</B></TH></TR>' +
	   '<TR><TD><B><span style="color:#B8312F;">Tarifa:</B></TD>' +
	   '<TD><Tarifa></TD></TR>' +
	   '<TR><TD><b><span style="color:#B8312F;">Marca:</b></TD>' +
	   '<TD><Marca></TD></TR>' +
	   '</table><br>En el PDF adjunto esta el listado'
    ...
    ;WITH Tabla AS
	   ...
	   SELECT   t1.CodEmpresa
			 ...
			 , REPLACE(REPLACE(@html
				,'<Tarifa>',CONCAT(COALESCE(TRIM([001]),''),COALESCE('.-'+TRIM([002]),'')))
				,'<Marca>',CONCAT(COALESCE(TRIM([003]),''),COALESCE('.-'+TRIM([004]),''),' (',TRIM([005]),' articulos)')) datRespuesta
	   FROM PivotTable t1
