...
-- Description: Transformar Reportes 113, 114 y 115 a HTML
-- =============================================
CREATE   PROCEDURE [dbo].[uspHTML_ReportesVisita] 
(
@CodPlantillaReportes nvarchar(20)
)
AS
BEGIN
    ...
    -- Plantilla HTML
     SELECT @html='<html><style>' +
	   ...
	   '<TR><TH colspan="2"><B>Reporte de Visita <TipoVisita></B></TH></TR>' +
	   '<TR><TD><B><span style="color:#B8312F;">Cliente:</B></TD>' +
	   '<TD><Cliente> (<Poblacion>)</TD></TR>' +
	   '<TR><TD><b><span style="color:#B8312F;">Fecha Visita:</b></TD>' +
	   '<TD><FechaVisita> (<Vendedor>)</TD></TR>' +
	   '<TR><TD><b><span style="color:#B8312F;">Tipo Visita:</b></TD>' +
	   '<TD><TipoVisita></TD></TR>' +
	   '<TR><TD><b><span style="color:#B8312F;">Comentario Visita:</b></TD>' +
    ...
    ;WITH Tabla AS
	   ...
	   SELECT   t1.CodEmpresa
			 ...
			 , 0 flaObligado
			 , 0 ordReportesLin
			 , REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(@html
				,'<ProximaVisita>', IIF(COALESCE([003],'')='','','<TR><TD><b><span style="color:#B8312F;">Fecha Pr&oacute;xima Visita:&nbsp;</b></TD><TD>'+[003]+'</TD></TR>'))
				,'<ComentarioVisita>',COALESCE(UPPER([002]),''))
				,'<TipoVisita>',CONCAT(COALESCE([001],''),' (',t1a.desPlantillaReportes,')'))
				,'<FechaVisita>',FORMAT(t2.fecReporte,'dd/MM/yyyy HH:mm'))
