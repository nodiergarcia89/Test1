...
-- ===================================================================================
CREATE   PROCEDURE [dbo].[usprptAgendaCoffeeTester] 
      @RangoDias int=365
    , @Test int=0
AS
BEGIN
    ...
    -- Declarar variables
    declare @ClientesADias int=30
    declare @ClientesBDias int=90
    declare @ClientesCDias int=180
    declare @KgCambioMuelas int=450
    declare @CambioFiltro int=365

    if @Test=1 -- En depuracion cojo los datos de la ultima ejecucion
	   begin
		  select * from rpt.AgendaCoffeeTester
		  return
	   end
    -- Borrar datos
    TRUNCATE TABLE rpt.AgendaCoffeeTester

    -- Recuperar los datos de los clientes
    INSERT INTO rpt.AgendaCoffeeTester
    select *
    ...
    where DiasDesdeCoffeTester >= (CASE ClasifABC 
								WHEN 'A' THEN @ClientesADias
								WHEN 'B' THEN @ClientesBDias
								WHEN 'C' THEN @ClientesCDias
